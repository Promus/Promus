...
4. Commits that fix a bug in a previous commit should:
    1. If existing commit is already upstream: Start with `fixup!` and then the summary line of the commit it fixes. If you commit from a terminal you can use `git commit --fixup=«commit»`
...
2.	Clone to computer: `git clone git@github.com:«github account»/promus.git`
3.	cd into your repo: `cd promus`
4.	Set up remote upstream: `git remote add -f upstream git://github.com/promus/promus.git`
